...
def fetchBalanceSheetAnnual(stockList):
    for stock in stockList:
        # stock = stockList[0]
        balance_sheet_soup = BeautifulSoup(getHtml(
            f'https://www.investing.com/instruments/Financials/changereporttypeajax?action=change_report_type&pair_ID={stock.id}&report_type=BAL&period_type=Annual'),
            'html.parser')
